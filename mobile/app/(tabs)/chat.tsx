...
import { Users, Music, Check, MessageSquare, LogIn } from "lucide-react-native";
  ...
  return (
    <SafeAreaView className="flex-1 bg-[#121212]">
      ...
      <View className="flex-row items-center gap-2 px-4 py-3.5 border-b border-zinc-900/50 bg-[#121212]">
        ...
        <Text className="text-white text-base font-black tracking-tight">친구 활동 및 채팅</Text>
      </View>

      {!isSignedIn ? (
        // 비인증 플레이스홀더 뷰
        <View className="flex-1 items-center justify-center p-6 text-center">
          <View className="w-16 h-16 bg-zinc-800/50 rounded-full items-center justify-center mb-5 border border-zinc-800">
            <Users size={28} className="text-zinc-400" />
          </View>
          <Text className="text-white font-bold text-base mb-1.5">친구들의 음악 듣기</Text>
          <Text className="text-zinc-400 text-xs text-center max-w-[240px] leading-relaxed mb-6">
            로그인하면 친구들의 실시간 감상 활동을 확인하고, 1:1 메시지를 주고받을 수 있습니다.
          </Text>
          <TouchableOpacity
            onPress={() => router.push("/auth/login")}
            className="flex-row items-center gap-2 px-6 py-3 bg-emerald-500 rounded-full shadow-lg active:scale-95"
          >
            <LogIn size={16} color="black" />
            <Text className="text-black text-sm font-bold">로그인하기</Text>
          </TouchableOpacity>
        </View>
      ) : (
        <ScrollView className="flex-1" contentContainerStyle={{ paddingBottom: 130 }}>
          {isLoading && users.length === 0 ? (
            ...
            <View className="py-16 items-center">
              <Text className="text-zinc-500 text-xs">활동 중인 친구가 없습니다.</Text>
            </View>
          ) : (
            <View className="divide-y divide-zinc-900/30">
              {filteredUsers.map((friend) => {
                ...
                const isListening = isOnline && activityInfo?.status === "Listening";
                      ...
                      <View className="flex-1">
                        <View className="flex-row items-center gap-1.5">
                          <Text
                            numberOfLines={1}
                            className={`text-sm ${
                              isUnread ? "text-emerald-400 font-bold" : "text-white font-semibold"
                            }`}
                          >
                            {friend.fullName}
                          </Text>
                        ...
                        {isListening ? (
                          <Text numberOfLines={1} className="text-emerald-400 text-xs mt-0.5 font-medium">
                            {currentSong}
                          </Text>
                        ) : (
