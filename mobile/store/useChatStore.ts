...
export const useChatStore = create<ChatStore>((set, get) => ({
  ...
  connectSocket: (clerkId: string) => {
    ...
    // axios baseURL 주소를 기반으로 호스트 IP 소켓 주소 자동 매핑 (예: http://192.168.0.5:3000)
    const socketURL = axiosInstance.defaults.baseURL?.replace('/api', '') || "http://localhost:3000";
    ...
    socket.on("connect", () => {
      console.log("Socket connected successfully on mobile!");
    });
